...
def is_post_processed(post_link: str) -> bool:
    """Проверяет, обрабатывался ли уже пост по ссылке"""
    conn = get_db()
    cursor = conn.cursor()
    result = cursor.execute('SELECT 1 FROM processed_posts WHERE post_link = ?', (post_link,)).fetchone()
    conn.close()
    return bool(result)

def mark_post_processed(post_link: str):
    """Отмечает пост как обработанный по ссылке"""
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('INSERT OR IGNORE INTO processed_posts (post_link) VALUES (?)', (post_link,))
    conn.commit()
    conn.close()
...
async def handle_group_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ САМЫЙ ПЕРВЫЙ ЛОГ — ВСЕГДА СРАБАТЫВАЕТ
    logging.info("🔍 ===== handle_group_message ВЫЗВАН =====")
    
    if not update.message:
        logging.info("❌ update.message = None")
        return
    
    # ✅ ЛОГИРУЕМ ВСЁ, ЧТО ПРИШЛО
    msg = update.message
    logging.info(f"📩 chat_id: {msg.chat_id}")
    logging.info(f"📩 message_id: {msg.message_id}")
    logging.info(f"📩 text: {msg.text if msg.text else 'НЕТ ТЕКСТА'}")
    logging.info(f"📩 sender_chat: {msg.sender_chat.id if msg.sender_chat else 'НЕТ'}")
    logging.info(f"📩 photo: {'ЕСТЬ' if msg.photo else 'НЕТ'}")
    logging.info(f"📩 video: {'ЕСТЬ' if msg.video else 'НЕТ'}")
    logging.info(f"📩 document: {'ЕСТЬ' if msg.document else 'НЕТ'}")
    logging.info(f"📩 caption: {msg.caption if msg.caption else 'НЕТ ПОДПИСИ'}")
    
    # Проверка группы
    if msg.chat_id != GROUP_ID:
        logging.info(f"❌ chat_id {msg.chat_id} != {GROUP_ID}")
        return
    
    # Проверка sender_chat
    if not msg.sender_chat:
        logging.info("❌ Нет sender_chat (сообщение от пользователя, а не от канала)")
        return
    
    if msg.sender_chat.id != CHANNEL_ID:
        logging.info(f"❌ sender_chat.id {msg.sender_chat.id} != {CHANNEL_ID}")
        return
    
    post_id = str(msg.message_id)
    post_link = msg.link
    
    logging.info(f"✅ ВСЕ ПРОВЕРКИ ПРОЙДЕНЫ! post_id={post_id}, post_link={post_link}")
    
    if not post_link:
        logging.info(f"⚠️ Нет ссылки на пост {post_id}, пробуем получить по-другому...")
        # Для медиа иногда link = None, но можно получить через chat_id и message_id
        post_link = f"https://t.me/c/{str(GROUP_ID)[4:]}/{post_id}"
        logging.info(f"🔄 Создана ссылка вручную: {post_link}")
    ...
    try:
        ...
        save_comment(0, post_id, sent_msg.message_id, GROUP_ID, post_link)
        logging.info(f"✅ Ответ на сообщение {post_id} отправлен!")
    except Exception as e:
...
def main():
    ...
    MessageHandler(
        filters.Chat(chat_id=GROUP_ID),  # ← ЛОВИТ ВСЁ (текст, фото, видео, файлы)
