...
CHANNEL_ID = -1004452867464
GROUP_ID = -1004294922207
OWNER_ID = 5265325991
...
def init_db():
    ...
    # Создаём таблицу до миграций: на новой базе ALTER TABLE не должен
    # молча скрывать ошибку "no such table".
    cursor.execute('''
...
def ban_user(user_id: int, ban_until: Optional[datetime] = None, banned_by: int = None, reason: str = None):
    conn = get_db()
    cursor = conn.cursor()
    ban_until_value = ban_until.isoformat() if isinstance(ban_until, datetime) else ban_until
    cursor.execute('INSERT OR REPLACE INTO bans (user_id, ban_until, banned_by, reason) VALUES (?, ?, ?, ?)',
                   (user_id, ban_until_value, banned_by, reason))
    conn.commit()
    conn.close()
...
def is_banned(user_id: int) -> bool:
    ...
    try:
        ...
        # Некорректная запись не должна случайно открыть доступ забаненному.
        logging.exception("Некорректная дата бана для пользователя %s", user_id)
        return True
...
def parse_ban_args(args: list) -> tuple:
    ...
    if args[0].endswith('d') and args[0][:-1].isdigit() and int(args[0][:-1]) > 0:
        days = int(args[0][:-1])
...
async def whois(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        link = arg
        search_display = f"по <a href='{escape_html(link)}'>комментарию</a>"
        ...
        author_id = int(arg)
        search_display = f"по ID: <code>{escape_html(author_id)}</code>"
    
    elif arg.startswith('@'):
        try:
            user = await context.bot.get_chat(arg)
            author_id = user.id
            search_display = f"по юзернейму <code>{escape_html(arg)}</code>"
        except Exception as e:
            await update.message.reply_text(
    ...
    try:
        ...
        username = escape_html(f"@{user.username}" if user.username else "Не указан")
        full_name = escape_html(f"{user.first_name or ''} {user.last_name or ''}".strip() or "Не указано")
        ...
        if ban_info:
            ...
            reason = escape_html(ban_info.get('reason') or 'Не указана')
            ...
            if banned_by:
                try:
                    ...
                    admin_name = escape_html(
                        f"@{admin.username}" if admin.username else admin.first_name or str(banned_by)
                    )
                except:
                    admin_name = escape_html(banned_by)
            else:
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"📝 <b>Ответ:</b>\n<blockquote>{escape_html(reply_text)}</blockquote>\n\n"
                f"──────\n"
                f"<blockquote>Автор: {escape_html(reply_emoji)}</blockquote>\n\n"
                f"📎 <a href='{escape_html(comment_link)}'>Посмотреть комментарий</a>"
            ),
            parse_mode='HTML',
...
async def check_expired_bans(context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute('''
        ...
        WHERE ban_until IS NOT NULL AND datetime(ban_until) <= datetime(?)
    ''', (now.isoformat(),))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("write_comment"):
        ...
        if len(parts) != 2 or not parts[1].isdigit():
            await query.edit_message_text("❌ Некорректная ссылка на пост.")
            return
        post_id = parts[1]
        ...
        if len(parts) != 3 or not parts[1].isdigit() or not parts[2].isdigit():
            await query.edit_message_text("❌ Некорректная ссылка на комментарий.")
            return
        ...
        if comment_id != 0 and (
            not comment
            or str(comment["post_id"]) != str(post_id)
            or comment["group_id"] != GROUP_ID
        ):
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text or not text.strip():
        ...
        return
    if len(text) > 3500:
        await update.message.reply_text("❌ Сообщение слишком длинное. Максимум — 3500 символов.")
        return
    ...
    try:
        if state["action"] == "new_comment":
            ...
            formatted_text = (
                f"<blockquote>{escape_html(text)}</blockquote>\n\n"
                f"──────\n"
...
async def handle_deep_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.message.text or not update.message.text.startswith('/start'):
        return
    ...
    if param.startswith('comment_'):
        post_id = param[len('comment_'):]
        if not post_id.isdigit():
            await update.message.reply_text("❌ Некорректная ссылка на пост.")
            return
        ...
        payload = param[len('reply_'):]
        parts = payload.split('_')
        if len(parts) != 2 or not parts[0].isdigit() or not parts[1].isdigit():
            await update.message.reply_text("❌ Некорректная ссылка на комментарий.")
            return
...
def main():
    ...
    if not BOT_TOKEN:
        raise RuntimeError(
            ...
            "с новым токеном, полученным через BotFather."
        )
